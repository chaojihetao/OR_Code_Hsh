function [A,As,b,c,X,z]=BigMMethod(As,Ag,b,c)
c=c';
M=99999;
s=length(c);
if ~isempty(Ag)
    c=[c,zeros(1,length(Ag(:,1)))];
    As=[As,zeros(length(As(:,1)),length(Ag(:,1)))];
    Ag=[Ag,-eye(length(Ag(:,1)))];
    m1=0;
    k=zeros(1,length(Ag(1,:)));
    for j=1:length(Ag(1,:))
        n=0;
        for i=1:length(Ag(:,1))
            if Ag(i,j)~=0
