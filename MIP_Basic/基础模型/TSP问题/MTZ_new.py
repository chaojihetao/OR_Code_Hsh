...
def solve(data, solution):
    ...
    # 约束条件
    for i in range(data.nodeNum):
        model.addConstr(quicksum(x[i, j] for j in range(data.nodeNum)) == 1)

    for i in range(data.nodeNum):
        model.addConstr(quicksum(x[j, i] for j in range(data.nodeNum)) == 1)
...
def drawGraph(data, solution):
    ...
    # 绘制点集
    plt.scatter(data.cor_x[:], data.cor_y[:], color='blue', alpha=1, marker='o', linewidths=1, label='Node')
