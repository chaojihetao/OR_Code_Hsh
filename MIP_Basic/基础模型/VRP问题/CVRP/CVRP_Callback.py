...
# -*- coding:utf-8 -*-
@Project : CVRP_MTZ.py
@File : CVRP_Callback.py
@Author : Hsh
@Time : 2023-01-29 17:14
...
def subtourelim(model, where):
    if where == GRB.Callback.MIPSOL:
        ...
        for k in range(data.vehicleNum):
            unvisited = list(set([j for i, j, k in selected.select('*', '*', k)]))
            max_len = len(unvisited)
            cycle = unvisited
            while unvisited:  # true if list is non-empty
                ...
                while neighbors:
                    ...
                    neighbors = [j for i, j, k in selected.select(current, '*', k) if j in unvisited]
                if len(thiscycle) <= max_len:
                    cycle = thiscycle  # New shortest sub tour
                tour = cycle
                if len(tour) < max_len:
                    # add subtour elimination constr. for every pair of cities in subtour
                    model.cbLazy(quicksum(model._vars[i, j, k] for i in tour for j in tour if i != j) <= len(tour) - 1)


def solver(data, solution):
    # 建立模型
    model = Model('CVRP_SubTour')
    ...
    for k in range(data.vehicleNum):
        # ---流平衡约束---
        # 起点与终点的出入度
        model.addConstr(quicksum(x[0, j, k] for j in range(1, data.nodeNum)) == 1, name='const_0j')
        model.addConstr(quicksum(x[j, 0, k] for j in range(1, data.nodeNum)) == 1, name='const_j0')
        # 中间点出入度平衡
        for h in range(1, data.nodeNum):
            model.addConstr(quicksum(x[i, h, k] for i in range(data.nodeNum))
                            == quicksum(x[h, i, k] for i in range(data.nodeNum)))
        # 防止原地转圈
        for i in range(data.nodeNum):
            for j in range(data.nodeNum):
                if i == j:
    ...
    model.update()
    model._vars = x     # 获取变量
    model.Params.lazyConstraints = 1    # 设置参数
    model.optimize(subtourelim)     # CallBack方法求解
