    ...
    while True:
        ...
        for s in range(S):
            if s == 0:
                ...
                SP.update()
                # set dual objective
                obj = quicksum(apply_N[j] * D[j] for j in range(J)) - quicksum(d_hat[i, s] * E[i] for i in range(I)) - G * A
                SP.setObjective(obj, GRB.MAXIMIZE)
                ...
                # update objective
                new_obj = quicksum(apply_N[j] * D[j] for j in range(J)) - quicksum(d_hat[i, s] * E[i] for i in range(I)) - G * A
        ...
        if (benders_gap <= 0.01) or (bad_gap == 5):
            break
